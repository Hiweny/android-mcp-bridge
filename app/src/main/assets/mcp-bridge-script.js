...
(function () {
  ...
  const PANEL_CSS = `
    ...
    @media (max-width: 768px) {
      #mcp-fab { width:40px; height:40px; font-size:18px; }
      #mcp-panel { width:calc(100vw - 16px) !important; max-height:calc(100vh - 80px) !important; border-radius:12px; }
      #mcp-panel .hd { padding:10px 14px; }
      #mcp-panel .hd h3 { font-size:14px; }
      #mcp-tabs button { padding:8px 10px; font-size:11px; }
      .mcp-bd { padding:10px 12px; }
      .mcp-btn { padding:8px 14px; font-size:13px; min-height:36px; }
      .mcp-input { font-size:14px; padding:10px 12px; min-height:40px; }
      .mcp-sel { font-size:14px; padding:9px 10px; min-height:40px; }
      .mcp-tool { padding:9px 6px; }
      .mcp-toggle-row { padding:10px 0; }
      .mcp-switch { width:40px; height:22px; }
      .mcp-switch .slider:before { height:18px; width:18px; }
      .mcp-switch input:checked + .slider:before { transform:translateX(18px); }
      .mcp-result { max-height:200px; font-size:11px; }
      .ext-card { padding:10px; }
      .ext-card-actions .mcp-btn { font-size:12px; padding:6px 12px; min-height:32px; }
      #mcp-panel .hd .ver { font-size:10px; }
    }
  ...
  waitForDOM().then(() => {
    ...
    function posPanel() {
      ...
      if (isMob) {
        // Mobile: open downward from FAB, near-fullscreen
        const gap = 6;
        panel.style.top = (r.bottom + gap) + 'px';
        panel.style.bottom = 'auto';
        panel.style.maxHeight = (window.innerHeight - r.bottom - gap - 16) + 'px';
      } else {
        // Desktop: try to place panel above the FAB
        const gap = 10;
